...
def train_vae_epoch(epoch, args, rnn, output, data_loader,
    ...
    for batch_idx, data in enumerate(data_loader):
        ...
        if epoch % args.epochs_log == 0 and batch_idx == 0:  # only output first batch's statistics
            ...
            print('z_mu_mean', z_mu_mean, 'z_mu_min', z_mu_min, 'z_mu_max', z_mu_max,
                  'z_sgm_mean', z_sgm_mean, 'z_sgm_min', z_sgm_min, 'z_sgm_max', z_sgm_max)

        # logging
        log_value('bce_loss_'+args.fname,
                  loss_bce.data[0], epoch*args.batch_ratio+batch_idx)
        log_value('kl_loss_' + args.fname,
                  loss_kl.data[0], epoch*args.batch_ratio + batch_idx)
...
def test_vae_epoch(epoch, args, rnn, output, test_batch_size=16, save_histogram=False, sample_time=1):
    ...
    y_pred = Variable(torch.zeros(test_batch_size, max_num_node,
...
def train_mlp_epoch(epoch, args, rnn, output, data_loader,
    ...
    for batch_idx, data in enumerate(data_loader):
        ...
        # logging
        log_value('loss_'+args.fname, loss.item(),
                  epoch*args.batch_ratio+batch_idx)
    ...
    for batch_idx, data in enumerate(data_loader):
        ...
        for j in range(y.size(1)):
            ...
            loss += binary_cross_entropy_weight(
                y_pred[:, j, 0:end_idx], y[:, j, 0:end_idx])*end_idx
...
def train(args, dataset_train, rnn, output):
    ...
    if args.load:
        fname = args.model_save_path + args.fname + \
            'lstm_' + str(args.load_epoch) + '.dat'
        rnn.load_state_dict(torch.load(fname))
        fname = args.model_save_path + args.fname + \
            'output_' + str(args.load_epoch) + '.dat'
...
def train_graph_completion(args, dataset_test, rnn, output):
    ...
    for sample_time in range(1, 4):
        ...
        # save graphs
        fname = args.graph_save_path + args.fname_pred + \
            str(epoch) + '_'+str(sample_time) + 'graph_completion.dat'
        save_graph_list(G_pred, fname)
